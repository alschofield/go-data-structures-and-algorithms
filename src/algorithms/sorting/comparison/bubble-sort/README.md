...
stable, and implement early exit. Empty/singleton input is a successful no-op.
Success returns `(true, nil)`. A nil comparator returns
`(false, ErrNilComparator)` without changing input. Do not call `sort`.
