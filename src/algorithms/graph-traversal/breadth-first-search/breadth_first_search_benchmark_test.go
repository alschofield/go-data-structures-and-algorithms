...
func BenchmarkBreadthFirstSearch(b *testing.B) {
	for _, size := range []int{256, 1_024} {
		list, source_key := bfsStarGraph(b, size)

		b.Run(fmt.Sprintf("full-star/Size=%d", size), func(b *testing.B) {
			b.ReportAllocs()
			for iteration := 0; iteration < b.N; iteration++ {
				path, complete, err := BreadthFirstSearch(list, source_key, func(*graph.Node[int]) bool { return true })
				if err != nil || !complete || len(path) != size {
					b.Fatal("full BFS traversal failed")
				}
			}
		})
		...
		b.Run(fmt.Sprintf("early-stop/Size=%d", size), func(b *testing.B) {
			b.ReportAllocs()
			for iteration := 0; iteration < b.N; iteration++ {
				path, complete, err := BreadthFirstSearch(list, source_key, func(node *graph.Node[int]) bool { return node.Key != size/2 })
				if err != nil || !complete || len(path) != size/2+1 {
					b.Fatal("early-stop BFS traversal failed")
				}
			}
		})
	}
...
func bfsStarGraph(b *testing.B, size int) (*adjacency_list.AdjacencyList[int], int) {
